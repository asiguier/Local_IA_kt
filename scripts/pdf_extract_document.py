...
# ── Conversion ────────────────────────────────────────────────────
def convertir_page_en_markdown(page):
    lignes_md = []
    ...
    if mots:
        premier = mots[0]["text"].strip()
        if est_titre_principal(premier):
            lignes_md.append(f"# {premier}\n")
...
def main():
    ...
    with pdfplumber.open(PDF) as pdf:
        ...
        for i, page in enumerate(pdf.pages):
            ...
            contenu = convertir_page_en_markdown(page)
            if contenu.strip():
